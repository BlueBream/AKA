...
<div class="page-header position-relative">
	...
	<div class="row-fluid">
		<form method="post" action='<?php echo base_url();?>c_index_aka/<?php echo $location; ?>'>
		...
		<div class="controls">
			<?php foreach($mk as $get_id){ ?>
				...
				<input type="text" name="id_mk" autocomplete = "off" readonly value="<?php echo $get_id['kode_mk']; ?>">
			<?php } ?>
		</div>
		<div class="controls">
		...
		<?php 
			foreach($frs as $frs){
		?>
			<input type="hidden" name="semester" value="<?php echo $frs['semester']; ?>">
		<?php
			}
		 ?>
		<?php foreach($nilai as $nilai){ ?>
			...
			<input type="hidden" name="id_mk" value="<?php echo $this->uri->segment(4);?>">	
				<label class="control-label" for="form-field-1">Nilai UTS andry</label>
